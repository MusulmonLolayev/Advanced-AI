...
\medskip
\textbf{Practical usage now:} interpretable tabular classification, simple rule extraction, baseline supervised modeling, and preparation for random forests and boosting.
...
\begin{itemize}
  ...
  \]
  \item Pure and impure nodes:
  \[
    I_G(D_m)=0
    \quad\text{means the node is pure,}
  \]
  while larger impurity means that several classes are mixed in the same node.
...
\section*{Roadmap}
\begin{itemize}
  \item First prepare and verify the datasets.
  \item Then implement the tree core in small pieces: impurity, split scoring, split search, recursion, and prediction.
  \item Run self-checks before using the task runners.
  \item Complete the toy, Iris, and overfitting studies in order.
\end{itemize}
...
\begin{itemize}
  ...
  \item Include one PDF report with equations, plots, tables, and conclusions.
  \item Include random seed, environment file, and reproducibility notes.
  \item Include outputs from \texttt{toy\_tree.py}, \texttt{iris\_tree.py}, and \texttt{depth\_and\_overfitting.py}.
  \item Clearly separate hand calculations, implementation checks, and experimental conclusions.
\end{itemize}
