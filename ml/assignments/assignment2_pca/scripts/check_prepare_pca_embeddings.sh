...
for arg in "$@"; do
  ...
  if [[ "${arg}" == "--core-only" || "${arg}" == "--skip-embeddings" ]]; then
    echo "check_prepare_pca_embeddings.sh cannot be used with ${arg} because it verifies text embeddings." >&2
    exit 1
  fi
done
...
embeddings_path = assignment_root / "datasets" / "pca_text_embeddings.npz"

if not embeddings_path.exists():
    raise FileNotFoundError(f"Missing prepared embeddings: {embeddings_path}")

with np.load(embeddings_path, allow_pickle=True) as bundle:
    required = {
        "query_embeddings",
        "doc_embeddings",
        "relevance",
    ...
    if missing:
        raise KeyError(f"Embedding bundle missing keys: {missing}")
...
print(f"Verified: {embeddings_path}")
PY
