"""Starter data utilities for Assignment 2: PCA."""
...
def load_npz_bundle(path: str | Path, required_keys: list[str]) -> dict[str, np.ndarray]:
    """Public wrapper for loading a named NPZ bundle."""
...
def load_pca_core_bundle(path: str | Path) -> dict[str, np.ndarray]:
    """Load the tiny PCA warm-up bundle."""
    return _load_required_npz(path, ["X"])


def load_text_embeddings_bundle(path: str | Path) -> dict[str, np.ndarray]:
    """Load the synthetic text-embedding bundle used in Lab 2."""
    required = [
        "query_embeddings",
        "doc_embeddings",
        "relevance",
...
def load_pca_knn_bundle(path: str | Path) -> dict[str, np.ndarray]:
    """Load the PCA + k-NN comparison bundle."""
