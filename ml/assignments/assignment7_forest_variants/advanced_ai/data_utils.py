"""Dataset helpers for Assignment 7."""
...
def load_isolation_bundle(path: str | Path) -> dict[str, np.ndarray]:
    return _load_required_npz(path, ["X", "y_outlier", "feature_names"])


def load_extra_trees_bundle(path: str | Path) -> dict[str, np.ndarray]:
    return _load_required_npz(path, ["X", "y", "feature_names", "target_names"])


def load_quantile_bundle(path: str | Path) -> dict[str, np.ndarray]:
    return _load_required_npz(path, ["X_train", "y_train", "X_test", "y_test", "feature_names"])
