...
\begin{itemize}
  \item Features $X=\{x_1,\dots,x_n\}$, $x_i\in\mathbb{R}^d$, and labels $y=\{y_1,\dots,y_n\}$, $y_i\in\{1,\dots,C\}$.
  \item Bootstrap sample $(X^{(b)},y^{(b)})$: $n$ draws uniformly with replacement from $(X,y)$.
  \item OOB probability:
  \[
  ...
  \]
  \item Tree impurity and split: same as in Lab 5 (Gini impurity $I_G$, impurity decrease $\Delta I$, threshold search).
