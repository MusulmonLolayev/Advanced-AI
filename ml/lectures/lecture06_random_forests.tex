...
\begin{itemize}
  ...
  \pause
  \item The probability that a particular example is left out is
  \[
    P(\text{example }i\notin D^{(b)})=\Bigl(1-\frac{1}{n}\Bigr)^{\!n}\approx \frac{1}{e}\approx 0.368,
  \]
...
\column{0.53\textwidth}
\begin{itemize}
  \item As shown in Eq.~\ref{eq:bootstrap-sample}, each bootstrap draw leaves out about $37\%$ of the original examples.
  \pause
