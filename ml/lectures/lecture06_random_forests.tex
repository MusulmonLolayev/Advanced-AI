...
\begin{itemize}
  \item Features $X=\{x_1,\dots,x_n\}$, labels $y$.
  \pause
  \item For each $b$, draw $n$ examples with replacement: bootstrap $(X^{(b)},y^{(b)})$.
  \pause
  \item Omission probability per example:
  \begin{equation}
    P\bigl((x_i,y_i)\notin (X^{(b)},y^{(b)})\bigr)=\Bigl(1-\frac{1}{n}\Bigr)^{\!n}\approx\frac{1}{e}.
    \label{eq:oob-probability}
  \end{equation}
  \pause
  \item Each bootstrap contains $\approx 63\%$ of the original examples; the remaining $37\%$ are out-of-bag.
\end{itemize}

\pause
\column{0.55\textwidth}
\centering
\includegraphics[width=0.62\textwidth]{figures/random_forests/bagging_diagram.pdf}
\end{columns}
\end{frame}

\begin{frame}{Training a Forest}
\begin{itemize}
  \item For each bootstrap sample $(X^{(b)},y^{(b)})$, train one full decision tree $T_b$.
  \pause
...
\begin{itemize}
  ...
  \end{equation}
  \pause
  \item In case of a tie, the class with the largest count wins.
\end{itemize}
...
\begin{frame}{Regression with Decision Trees}
\begin{itemize}
  \item Decision trees also handle regression when the target is continuous: $y_i\in\mathbb{R}$.
  \pause
  \item Split objective: minimize the mean squared error (MSE) in child nodes instead of Gini impurity.
  \pause
  \item For a candidate split $(j,t)$, the child MSE is
  \[
    \text{MSE}_L=\frac{1}{|D_L|}\sum_{(x_i,y_i)\in D_L}(y_i-\bar{y}_L)^2,\quad
    \text{MSE}_R=\frac{1}{|D_R|}\sum_{(x_i,y_i)\in D_R}(y_i-\bar{y}_R)^2,
  \]
  where $\bar{y}_L,\bar{y}_R$ are the mean target values in the left and right child.
  \pause
  \item The split chooses $(j,t)$ that minimizes the weighted average:
  \[
    \frac{|D_L|}{|D_m|}\text{MSE}_L + \frac{|D_R|}{|D_m|}\text{MSE}_R.
  \]
  \pause
  \item Leaf prediction for regression: the mean target value of examples in that leaf
  \[
    \hat{y} = \frac{1}{n_m}\sum_{(x_i,y_i)\in D_m} y_i.
  \]
\end{itemize}
\end{frame}
...
\column{0.53\textwidth}
\begin{itemize}
  \item Each bootstrap leaves out $\approx 37\%$ of examples (Eq.~\eqref{eq:oob-probability}). These ``out-of-bag'' (OOB) samples are unseen by that tree.
  \pause
  \item For each example $(x_i,y_i)$, aggregate predictions only from trees that did not see it:
  \[
    \hat{y}_{\text{OOB}}(x_i)
    =
    \arg\max_{c}
    \sum_{b:\;(x_i,y_i)\notin (X^{(b)},y^{(b)})}
    \mathbf{1}\{\hat{y}^{(b)}(x_i)=c\}.
  \]
  \pause
  \item OOB error = fraction of training examples where $\hat{y}_{\text{OOB}}(x_i)\neq y_i$.
\end{itemize}

\pause
\column{0.47\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{figures/random_forests/oob_illustration.pdf}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \pause
  \item For feature $j$, sum the impurity decrease over all nodes in all trees where $j$ was chosen as the split feature:
  \begin{equation}
    I_j
    =
    \frac{1}{B}
    \sum_{b=1}^{B}\;
    \sum_{\substack{m\in T_b\\ \text{feature }m=j}}
    \frac{n_m}{n}\,\Delta I,
    \label{eq:feature-importance}
  \end{equation}
where $n_m$ is the number of training examples at node $m$.
  \pause
  \item Normalize so $\sum_{j=1}^{d}I_j = 1$.
  \pause
  \item Higher $I_j$ means feature $j$ is used in more high-importance splits.
\end{itemize}
\end{frame}

\begin{frame}{Decision Boundary Comparison}
\centering
\includegraphics[width=0.7\textwidth]{figures/random_forests/boundary_comparison.pdf}
...
\centering
\includegraphics[scale=0.8]{figures/random_forests/error_vs_trees.pdf}
