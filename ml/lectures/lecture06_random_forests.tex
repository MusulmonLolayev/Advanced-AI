...
\begin{itemize}
  ...
  \pause
  \item For $b=1,\dots,B$, draw a bootstrap sample $D^{(b)}$ of size $n$ from $D$ uniformly with replacement.
  \pause
  \item Each $D^{(b)}$ has size $n$ but some examples appear multiple times, others are left out.
  \pause
  \item The probability that a given example is omitted is
  \begin{equation}
    P\bigl((x_i,y_i)\notin D^{(b)}\bigr)=\Bigl(1-\frac{1}{n}\Bigr)^{\!n}\approx\frac{1}{e}\approx 0.368.
