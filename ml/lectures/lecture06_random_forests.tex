...
\begin{itemize}
  \item Decision trees also handle regression: target $y_i\in\mathbb{R}$.
  \pause
  \item Split objective: minimize MSE instead of Gini impurity. For a split $(j,t)$:
  \[
    \text{MSE}_L=\frac{1}{|D_L|}\sum_{(x_i,y_i)\in D_L}(y_i-\bar{y}_L)^2,\qquad
    \text{MSE}_R=\frac{1}{|D_R|}\sum_{(x_i,y_i)\in D_R}(y_i-\bar{y}_R)^2,
  \]
  where $\bar{y}_L,\bar{y}_R$ are the child means.
  \pause
  \item Choose $(j,t)$ minimizing the weighted average $\displaystyle\frac{|D_L|}{|D_m|}\text{MSE}_L+\frac{|D_R|}{|D_m|}\text{MSE}_R$.
  \pause
  \item Leaf prediction: the mean target in that leaf $\displaystyle\hat{y}=\frac{1}{n_m}\sum_{(x_i,y_i)\in D_m}y_i$.
\end{itemize}
\end{frame}
