...
\begin{itemize}
  \item Start with feature matrix $X\in\mathbb{R}^{n\times d}$ and labels $y$.
  \pause
  \item For $b=1,\dots,B$, draw $n$ indices uniformly with replacement:
  \[
    I^{(b)} = \{i_1^{(b)},\dots,i_n^{(b)}\},\qquad
    i_k^{(b)}\in\{1,\dots,n\}.
  \]
  \pause
  \item The bootstrap training set is the rows of $(X,y)$ at those indices.
  \pause
  \item Many original examples appear multiple times; some are left out.
  \pause
  \item The probability that a particular example is omitted is
  \[
    P\bigl(i\notin I^{(b)}\bigr)=\Bigl(1-\frac{1}{n}\Bigr)^{\!n}
    \approx\frac{1}{e}\approx 0.368,
  \]
  so about $63\%$ of the original examples appear in each bootstrap sample.
\end{itemize}
...
\begin{itemize}
  \item For each bootstrap index set $I^{(b)}$, train one full decision tree $T_b$ on $(X_{I^{(b)}}, y_{I^{(b)}})$.
  \pause
...
\column{0.53\textwidth}
\begin{itemize}
  \item Each bootstrap draw $I^{(b)}$ omits about $37\%$ of the original examples (left panel).
  \pause
...
\begin{itemize}
  ...
  \end{equation}
  \pause
  \item Normalize so $\sum_{j=1}^{d}\mathcal{I}_j = 1$.
  \pause
  \item High $\mathcal{I}_j$ means feature $j$ is used in many high-impact splits.
\end{itemize}
\end{frame}
