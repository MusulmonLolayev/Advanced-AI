...
\begin{itemize}
  \item Start with features $X=\{x_1,\dots,x_n\}$, $x_i\in\mathbb{R}^d$, and labels $y=\{y_1,\dots,y_n\}$, $y_i\in\{1,\dots,C\}$.
  \pause
  \item For $b=1,\dots,B$, draw a bootstrap sample $(X^{(b)},y^{(b)})$ of size $n$ uniformly with replacement.
  \pause
  \item Each bootstrap sample has size $n$ but some examples appear multiple times, others are left out.
  \pause
  \item The probability that a given example is omitted is
  \begin{equation}
    P\bigl((x_i,y_i)\notin (X^{(b)},y^{(b)})\bigr)=\Bigl(1-\frac{1}{n}\Bigr)^{\!n}\approx\frac{1}{e}\approx 0.368.
    \label{eq:oob-probability}
  \end{equation}
  \pause
  \item Hence about $63\%$ of the original examples appear in each bootstrap sample.
\end{itemize}

\pause
\column{0.47\textwidth}
\centering
\includegraphics[width=0.75\textwidth]{figures/random_forests/bagging_diagram.pdf}
\end{columns}
\end{frame}
...
\column{0.53\textwidth}
\begin{itemize}
  \item As shown in Eq.~\eqref{eq:oob-probability}, each bootstrap draw leaves out about $37\%$ of the original examples.
  \pause
  \item These ``out-of-bag'' (OOB) samples are not used to train that tree.
  \pause
  \item For each training example $(x_i,y_i)$, let $\mathcal{B}_i=\{b:(x_i,y_i)\notin (X^{(b)},y^{(b)})\}$ be the trees where it is OOB.
  \pause
  \item The OOB prediction aggregates only those trees:
  \[
    \hat{y}_{\text{OOB}}(x_i)
    =
    \arg\max_{c}\sum_{b\in\mathcal{B}_i}\mathbf{1}\{\hat{y}^{(b)}(x_i)=c\}.
  \]
  \pause
  \item OOB error is the fraction of OOB predictions that are wrong.
\end{itemize}
