...
\begin{itemize}
  \item Draw a random subsample of size $n$ from the data (no class labels needed).
  \pause
...
\small
\begin{itemize}
  \item Subsample size $n=16$ for each isolation tree.
...
\begin{itemize}
  \item Average path length over an unsuccessful binary search tree of $n$ nodes:
  \begin{equation}
...
\small
\begin{itemize}
  \item For $n=16$: $H(15) \approx 3.3182$, so by Eq.~\eqref{eq:avg-path-length}
  \[
    c(16) = 2(3.3182) - \frac{2(15)}{16} \approx 4.7614.
  \]
  \pause
  \item Point $A$ (anomaly), $E[h(A)] = 2.0$:
  \[
    s(A,16) = 2^{-2.0/4.7614} = 2^{-0.420} \approx 0.747.
  \]
  \pause
  \item Point $B$ (normal), $E[h(B)] = 6.0$:
  \[
...
\begin{itemize}
  ...
  \pause
  \item A common rule: flag $x$ as an anomaly if $s(x,n)$ exceeds a chosen threshold (e.g.\ $0.6$--$0.7$).
\end{itemize}
\end{frame}
...
\column{0.53\textwidth}
\begin{itemize}
  \item A standard regression forest predicts only the conditional mean $E[y\mid x]$.
  \pause
...
\begin{itemize}
  ...
  \[
    \widehat{F}(y\mid x) = \sum_{i=1}^{n} w_i(x)\,\mathbf{1}\{y_i \le y\}.
  \]
  \pause
  \item The $\tau$-quantile prediction is $\widehat{Q}_\tau(x) = \inf\{y : \widehat{F}(y\mid x) \ge \tau\}$.
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \]
  \pause
  \item Median ($\tau=0.5$) prediction: $\widehat{Q}_{0.5}(x) = 16.5$.
  \pause
  \item A $80\%$ prediction interval uses $\tau=0.1$ and $\tau=0.9$:
  \[
    \widehat{Q}_{0.1}(x) = 14,\qquad \widehat{Q}_{0.9}(x) = 19.
  \]
  \pause
...
\begin{itemize}
  \item \textbf{Isolation Forest:} \texttt{n\_estimators} $B$, subsample size $n$ per tree (small, e.g.\ 256), height limit $\approx \log_2 n$.
  \pause
...
\begin{itemize}
  ...
  \item The anomaly score $s(x,n) = 2^{-E[h(x)]/c(n)}$ maps short paths to scores near 1.
...
\begin{itemize}
  ...
  \pause
  \item Compute $c(n)$ and the anomaly score $s(x,n)$ for a toy dataset with injected outliers.
  \pause
